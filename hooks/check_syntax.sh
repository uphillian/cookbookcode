...
do
  ...
  if [ $(git cat-file -s :0:$indexfile) -gt 0 ]; then
    case $indexfile in
    *.pp )
      # Check puppet manifest syntax 
      git cat-file blob :0:$indexfile | puppet parser validate 2>$error_msg ;;
    *.erb )
...
if [ "$syntax_errors" -ne 0 ]; then
  if [ "$syntax_errors" -gt 1 ]; then 
    echo "Error: $syntax_errors syntax errors found, aborting commit."
  else
    echo "Error: 1 syntax error found, aborting commit."
  fi
  exit 1
fi
